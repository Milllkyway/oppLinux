...
int main() {
    ...
    perKey = ftok(FILE_NAME, 1);
    cout << mainKey << endl;
    cout << perKey << endl;

    mainQueue = msgget(mainKey, IPC_CREAT | IPC_EXCL | 0040 | 0020); // group queue; 0040 = reading for group; 0020 = writing for group
    if (mainQueue == -1) {
    ...
    perQueue = msgget(perKey, IPC_CREAT | IPC_EXCL | 0400 | 0002); // personal queue; 0400 = reading for author; 0002 = writing for other
    if (perQueue == -1) {
    ...
    myMSG req = {8, NAME, mainQueue, reqTime, false};
    int send = msgsnd(req.queueID, &req, sizeof(myMSG), 0); // send request to read file
    ...
    while (read == false || others < 3) {
        ...
        if (getMsg.progNum != NAME) {
            resSend(1, getMsg.queueID, getMsg.progNum); // send response to getMsg.queueID
            others++;
        } else {
            // getResponse
            myMSG getMsg1, getMsg2;
            int resMsg1 = msgrcv(perQueue, &getMsg1, sizeof(myMSG), 12, 0);
            if (resMsg1 != -1) 
                cout << "Received response from program " << getMsg1.progNum << "; Time: " << getMsg1.sendTime << endl; 
            int resMsg2 = msgrcv(perQueue, &getMsg2, sizeof(myMSG), 13, 0);
            if (resMsg2 != -1) 
